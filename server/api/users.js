...
router.get('/', requireAdmin, (req, res, next) => {
	return User.findAll().then((users) => res.json(users)).catch(next);
});
...
router.get('/:id/orders', requireLogin, requireUserOrAdmin, (req, res, next) => {
...
router.post('/', (req, res, next) => {
	return User.create(underscore.pick(req.body, userFieldAllowList)).then((user) => res.json(user)).catch(next);
});
...
router.delete('/:id', requireLogin, requireUserOrAdmin, (req, res, next) => {
	return User.findById(req.params.id)
		.then((user) => {
			if (user) {
				return User.destroy({where: {id: req.params.id}}).then(() => res.sendStatus(202));
